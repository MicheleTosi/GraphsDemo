package it.micheletosi.graphsdemo.adapter.chart
...
import it.micheletosi.graphsdemo.R


class LineInsertDataAdapter : RecyclerView.Adapter<LineInsertDataAdapter.Holder>() {
    private var c = 1
    private val holders: MutableList<Holder>
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        //inflating the layout for insertion of the LineChart data
        val constraintLayout = LayoutInflater.from(parent.context)
            .inflate(R.layout.card_view_line_insert_data, parent, false) as ConstraintLayout
        return Holder(constraintLayout)
    }

    override fun onBindViewHolder(holder: Holder, position: Int) {
        holders.add(holder)
        val name = String.format("%s %d", holder.etLineName.hint.toString(), position+1)
        holder.etLineName.setText(name)
    ...
    val names: Array<String?>
        get() {
            ...
            for (i in 0 until c) {
                values[i] = holders[i].etLineName.text.toString()
            }
            return values
        }
...
//        val tvLineName: TextView = itemView.findViewById(R.id.tvNameLine)
...
//        private val tvYAxis: TextView = itemView.findViewById(R.id.tvYAxis)
        val etLineName: EditText = itemView.findViewById(R.id.etNameLine)
